...
public class S3SignerAWS  {
        ...
        throws -> [String:String]
    {
        ...
        if httpMethod == .put && payload.isData {
            updatedHeaders["content-md5"] = Insecure.MD5.hash(data: payload.data).description.data(using: .utf8)?.base64EncodedString()
        }
        ...
        throws -> String
    {
        ...
        let dateKey = HMAC<SHA256>.authenticationCode(for: timeStampShort.convertToData(), using: .init(data: "AWS4\(secretKey)".convertToData()))
        let dateRegionKey = HMAC<SHA256>.authenticationCode(for: region.rawValue.convertToData(), using: .init(data: dateKey))
        let dateRegionServiceKey = HMAC<SHA256>.authenticationCode(for: service.convertToData(), using: .init(data: dateRegionKey))
        let signingKey = HMAC<SHA256>.authenticationCode(for: "aws4_request".convertToData(), using: .init(data: dateRegionServiceKey))
        let signature = HMAC<SHA256>.authenticationCode(for: stringToSign.convertToData(), using: .init(data: signingKey)).hexEncodedString()
        return signature
    }
    ...
    internal func createStringToSign(
        ...
        throws -> String
    {
        let canonRequestHash = SHA256.hash(data: canonicalRequest.data(using: .utf8)!).hexEncodedString()
